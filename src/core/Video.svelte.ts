...
let renderingController = $state<AbortController | null>(null);
let renderingProgress = $state<{ stage: 'encode' | 'render'; progress: number }>({ stage: 'render', progress: 0 });
...
export const Video = {
  ...
  get renderingController() { return renderingController },
  set renderingController(value) { renderingController = value },
  get renderingProgress() { return renderingProgress },
  set renderingProgress(value) { renderingProgress = value },
  async encode(
